...
@foreach ($allAvgs["types"] as $defID => $avgs)
    <tr @if ($defID == 144) class="brdTop" @endif >
        ...
        <td class="brdRgt"> @if ($avgs["ovr"][0] > 0) {{ round($avgs["ovr"][1]/$avgs["ovr"][0]) }}% @endif </td>
        <td> @if ($avgs["fac"][0] > 0) {{ $GLOBALS["SL"]->sigFigs($avgs["fac"][1]/$avgs["fac"][0], 3) }} @endif </td>
        <td> @if ($avgs["pro"][0] > 0) {{ $GLOBALS["SL"]->sigFigs($avgs["pro"][1]/$avgs["pro"][0], 3) }} @endif </td>
        <td> @if ($avgs["lgt"][0] > 0) {{ $GLOBALS["SL"]->sigFigs($avgs["lgt"][1]/$avgs["lgt"][0], 3) }} @endif </td>
        <td> @if ($avgs["hvc"][0] > 0) {{ $GLOBALS["SL"]->sigFigs($avgs["hvc"][1]/$avgs["hvc"][0], 3) }} @endif </td>
...
@forelse ($allAvgs["zones"] as $zone => $avgs)
    @if ($avgs["tot"] > 1)
        <tr @if ($firstRow) class="brdTop" @endif ><?php $firstRow = false; ?>
            ...
            @if ($avgs["ovr"][0] > 0) <td class="brdRgt">{{ round($avgs["ovr"][1]/$avgs["ovr"][0]) }}%</td>
            @else <td class="brdRgt"> </td> @endif
...
@foreach (['PowerScore Light Types', 'PowerScore HVAC Systems'] as $set)
    ...
    @foreach ([ 162 => 'Flower', 163 => 'Veg', 160 => 'Clone'] as $area => $areaName)
        ...
        @foreach ($GLOBALS["SL"]->def->getSet($set) as $def)
            <tr>
                <th> @if ($set == 'PowerScore HVAC Systems') <a href="javascript:;" id="hidivBtnH{{ $def->DefID 
                    }}" class="hidivBtn"> {{ $areaName }} HVAC {!! str_replace(' - ', 
                    '</a> <div id="hidivH' . $def->DefID . '" class="disNon slGrey">', $def->DefValue) !!}</div> 
                    @else {{ $areaName }} {{ str_replace('double-ended ', '2x', str_replace('single-ended ', '1x', $def->DefValue)) }} @endif
                    </th>
                <td class="brdRgt"> @if ($allAvgs["avgs"][$area . '-' . $def->DefID]["ovr"][0] > 0) {{ 
                    round($allAvgs["avgs"][$area . '-' . $def->DefID]["ovr"][1]/$allAvgs["avgs"][$area . '-' 
                        . $def->DefID]["ovr"][0]) }}% @else <span class="slGrey">0%</span> @endif </td>
                <td> @if ($allAvgs["avgs"][$area . '-' . $def->DefID]["fac"][0] > 0) {{ 
                    $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$area . '-' . $def->DefID]["fac"][1]/$allAvgs["avgs"][$area 
                        . '-' . $def->DefID]["fac"][0], 3) }} @else <span class="slGrey">0</span> @endif </td>
                <td> @if ($allAvgs["avgs"][$area . '-' . $def->DefID]["pro"][0] > 0) {{ 
                    $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$area . '-' . $def->DefID]["pro"][1]/$allAvgs["avgs"][$area 
                        . '-' . $def->DefID]["pro"][0], 3) }} @else <span class="slGrey">0</span> @endif </td>
                <td> @if ($allAvgs["avgs"][$area . '-' . $def->DefID]["lgt"][0] > 0) {{ 
                    $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$area . '-' . $def->DefID]["lgt"][1]/$allAvgs["avgs"][$area 
                        . '-' . $def->DefID]["lgt"][0], 3) }} @else <span class="slGrey">0</span> @endif </td>
                <td> @if ($allAvgs["avgs"][$area . '-' . $def->DefID]["hvc"][0] > 0) {{ 
                    $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$area . '-' . $def->DefID]["hvc"][1]/$allAvgs["avgs"][$area 
                        . '-' . $def->DefID]["hvc"][0], 3) }} @else <span class="slGrey">0</span> @endif </td>
                <td class="slGrey brdLft">{{ number_format($allAvgs["avgs"][$area . '-' . $def->DefID]["ovr"][0]) 
                    }}</td>
            </tr>
        @endforeach
    @endforeach
...
@foreach ($GLOBALS["SL"]->def->getSet('PowerScore Onsite Power Sources') as $def)
    <tr @if ($firstRow) class="brdTop" @endif ><?php $firstRow = false; ?>
        <th><nobr>{{ $def->DefValue }}</nobr></th>
        <td class="brdRgt"> @if ($allAvgs["avgs"][$def->DefID]["ovr"][0] > 0) {{ 
            round($allAvgs["avgs"][$def->DefID]["ovr"][1]/$allAvgs["avgs"][$def->DefID]["ovr"][0]) 
            }}% @else <span class="slGrey">0</span> @endif </td>
        <td> @if ($allAvgs["avgs"][$def->DefID]["fac"][0] > 0) {{ 
            $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$def->DefID]["fac"][1]/$allAvgs["avgs"][$def->DefID]["fac"][0], 3) 
            }} @else <span class="slGrey">0</span> @endif </td>
        <td> @if ($allAvgs["avgs"][$def->DefID]["pro"][0] > 0) {{ 
            $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$def->DefID]["pro"][1]/$allAvgs["avgs"][$def->DefID]["pro"][0], 3) 
            }} @else <span class="slGrey">0</span> @endif </td>
        <td> @if ($allAvgs["avgs"][$def->DefID]["lgt"][0] > 0) {{ 
            $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$def->DefID]["lgt"][1]/$allAvgs["avgs"][$def->DefID]["lgt"][0], 3) 
            }} @else <span class="slGrey">0</span> @endif </td>
        <td> @if ($allAvgs["avgs"][$def->DefID]["hvc"][0] > 0) {{ 
            $GLOBALS["SL"]->sigFigs($allAvgs["avgs"][$def->DefID]["hvc"][1]/$allAvgs["avgs"][$def->DefID]["hvc"][0], 3) 
            }} @else <span class="slGrey">0</span> @endif </td>
        <td class="slGrey brdLft">{{ number_format($allAvgs["avgs"][$def->DefID]["ovr"][0]) }}</td>
    </tr>
@endforeach
